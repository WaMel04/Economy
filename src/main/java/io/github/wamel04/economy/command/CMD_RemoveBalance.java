...
public class CMD_RemoveBalance implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length < 3) {
            sender.sendMessage("§a/removebalance [닉네임] [화폐] [금액] §7- 재화를 지급합니다.");
            return false;
        }
        if (args.length == 3) {
            ...
            if (target != null) {
                try {
                    if (EconomyAPI.removeBalance(target.getUniqueId().toString(), currencyName, balance)) {
                        if (sender instanceof Player)
                            sender.sendMessage("§6" + target.getName() + "§a님에게서 §e" + formattedBalance + " " + currencyName + "§a만큼 차감했습니다.");
                    } else {
                        if (sender instanceof Player)
                            sender.sendMessage("§6" + target.getName() + "§a님의 잔액이 부족합니다.");
                    }
                } catch (Exception e) {
                ...
                CompletableFuture.runAsync(() -> {
                    ...
                    try {
                        if (EconomyAPI.removeBalance(newTarget.getUniqueId().toString(), currencyName, balance)) {
                            if (sender instanceof Player)
                                sender.sendMessage("§6" + newTarget.getName() + "§a님에게서 §e" + formattedBalance + " " + currencyName + "§a만큼 차감했습니다.");
                        } else {
                            if (sender instanceof Player)
                                sender.sendMessage("§6" + newTarget.getName() + "§a님의 잔액이 부족합니다.");
                        }
                    } catch (Exception e) {
