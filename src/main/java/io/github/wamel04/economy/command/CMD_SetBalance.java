...
public class CMD_SetBalance implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length < 3) {
            sender.sendMessage("§a/setbalance [닉네임] [화폐] [금액] §7- 재화를 설정합니다.");
            return false;
        }
        if (args.length == 3) {
            ...
            try {
                ...
                sender.sendMessage("§c금액의 포맷은 0이상의 유리수입니다.");
                return false;
            }
            ...
            if (target != null) {
                try {
                    EconomyAPI.setBalance(target.getUniqueId().toString(), currencyName, balance);

                    if (sender instanceof Player)
                        sender.sendMessage("§6" + target.getName() + "§a님의 §e" + currencyName + " §a보유량을 §e" + formattedBalance + "§a(으)로 설정했습니다.");
                } catch (Exception e) {
                ...
                CompletableFuture.runAsync(() -> {
                    ...
                    try {
                        EconomyAPI.setBalance(newTarget.getUniqueId().toString(), currencyName, balance);

                        if (sender instanceof Player)
                            sender.sendMessage("§6" + newTarget.getName() + "§a님의 §e" + currencyName + " §a보유량을 §e" + formattedBalance + "§a(으)로 설정했습니다.");
                    } catch (Exception e) {
